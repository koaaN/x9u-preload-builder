...
int install_embedded_su(pid_t *daemon_pid) {
  ...
  /*
   * Prefer /data/local/tmp only on the first pass. open/write/chmod are usually
   * allowed under Firefox seccomp; mount/setns are not and used to SIGSYS.
   * Avoid apex/adbd mntns work unless local install already succeeded and we
   * explicitly want a second path later.
   */
  int local_ok = install_local_su_client();
  int apex_ok = 0;
  if (!local_ok) {
    pr_warning("local su install failed errno=%d; try apex mount path\n",
               errno);
    apex_ok = write_embedded_su();
    if (!apex_ok) {
      pr_warning("apex su install failed errno=%d\n", errno);
    }
  } else {
    pr_success("local su binary ready path=%s\n", SU_LOCAL);
  }
  ...
  const char *daemon_path = local_ok ? SU_LOCAL : SU_DST;
  unlink(SU_SOCK);
  unlink(SU_LOG);
  pid_t pid = fork();
  if (pid == 0) {
    ...
    execl(daemon_path, "su", "--daemon", (char *)NULL);
  ...
  for (int i = 0; i < 50; i++) {
    if (access(SU_SOCK, F_OK) == 0 && probe_su_daemon(pid)) {
      pr_success("embedded su daemon ready pid=%d socket=%s path=%s "
                 "local=%d apex=%d\n",
                 pid, SU_SOCK, daemon_path, local_ok, apex_ok);
      return 1;
    }
    usleep(100000);
  }

  /* Even without a live daemon, a root-owned local su binary is useful. */
  if (local_ok || apex_ok) {
    pr_warning("su daemon not ready pid=%d; binary still installed local=%d "
               "apex=%d\n",
               pid, local_ok, apex_ok);
    errno = ETIMEDOUT;
    return local_ok || apex_ok;
  }
